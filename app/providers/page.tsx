...
export default function ProvidersPage() {
  ...
  const filtered=useMemo(()=>providers.filter(p=>(filter==="All"||p.service===filter)&&(`${p.name} ${p.service} ${p.bn} ${p.area}`).toLowerCase().includes(query.toLowerCase())),[query,filter]);
  return <main className="page-shell"><header className="site-header"><Link className="brand" href="/">Laksam <span>Local Service</span></Link><nav><Link href="/services">{x.navServices}</Link><Link href="/providers">{x.navProviders}</Link><Link href="/#how">{x.navHow}</Link></nav><div style={{display:"flex",alignItems:"center",gap:16}}><LanguageToggle onChange={setLanguage}/><Link className="header-cta" href="/register">{x.join}</Link></div></header><section className="listing-head"><div><p className="eyebrow">{x.eyebrow}</p><h1>{x.title}</h1><p>{x.desc}</p></div><div className="listing-search"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&setQuery(query.trim())} placeholder={x.placeholder}/><button onClick={()=>setQuery(query.trim())}>{x.search}</button></div></section><div className="filter-row">{filters.map(f=><button key={f} className={(filter===f?"filter-active ":"")} onClick={()=>setFilter(f)}>{f==="All"?x.all:f}</button>)}<button>{x.nearby}</button></div><section className="provider-grid">{filtered.map(p=><article className="provider-card" key={p.name}><div className="provider-avatar">{p.name[0]}</div><div className="provider-main"><div className="provider-title"><h3>{p.name}</h3><BadgeCheck size={17}/></div><span className="provider-service">{language==="bn"?p.bn:p.service}</span><p><MapPin size={14}/> {p.area}</p><div className="provider-meta"><span><Star size={14} fill="currentColor"/> {p.rating}</span><span>{p.jobs}</span><b>{x.from} {p.price}</b></div></div><Link className="card-arrow" href="/providers/rahim-electric-service"><ArrowRight size={18}/></Link></article>)}{filtered.length===0&&<div style={{gridColumn:"1/-1",padding:"48px 20px",textAlign:"center",color:"#64748b"}}>{x.empty}</div>}</section></main>;
}
